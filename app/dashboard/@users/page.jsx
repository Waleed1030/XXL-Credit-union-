        ...
        <div className={styles.firstrow}>
            <div className={styles.balances}>
              <div className='flex gap-10 md:flex-row flex-col'>
                <div className={styles.card}>
                  ...
                  <p>Total Balance</p>
                  <span className={styles.amount}>${user.t_balance}</span>
                  </div>
                  < div className={styles.icon}>< HiOutlineBanknotes size={32}/></div>
                </div>

              <div className={styles.card}>
                  <div className={styles.textbox}>
                  <p>Available Balance</p>
                  <span className={styles.amount}>${user.a_balance}</span>
                  </div>
                  < div className={styles.icon}><  HiMiniBanknotes size={32}/></div>
              </div>
            </div>

              <div className='flex gap-20 items-center text-[--accent] underline text-[16px] font-bold md:text-[20px] font-thin cursor-pointer'>

                <a href="/dashboard/history"><span>View Statement</span></a>
                <a href="/dashboard/transfer"><span>Transfer Funds</span></a>
              </div>
            ...
            <div className={styles.accountdetails}>
              <span className={styles.amount}>Account Details</span>
                ...
                <div className={styles.item} style={{ backgroundColor: "#F5F5F5" }}>
          ...
          <thead>
              ...
              {history.slice(-10).map((item)=>(
                ...
                <td>{item.status === 'pending' ? <Pending/> : 
                     item.status === 'processed' ? <Processed/> : <Cancelled/>}</td>
