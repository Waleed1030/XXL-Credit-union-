...
export const fetchUserData = async (username) => {
  ...
  try {
    ...
    const history = await Transaction.find({ username: id })
      .sort({ createdAt: "desc" })
      .exec();
...
export const fetchTransactions = async (q, page) => {
  ...
  try {
    ...
    const transactions = await Transaction.find({ username: { $regex: regex } })
      .sort({ createdAt: "desc" })
      .limit(ITEM_PER_PAGE)
      .skip(ITEM_PER_PAGE * (page - 1));
...
export const fetchCards = async (q, page) => {
  ...
  try {
    ...
    const cards = await Cards.find({ username: { $regex: regex } })
      .limit(ITEM_PER_PAGE)
      .skip(ITEM_PER_PAGE * (page - 1));
    return { count, cards };
  } catch (error) {
